...
## A function to reduce the sample size down to a specific number so that
# even though the differently filtered DGPs have different sample sizes
# (by definition, a selection of data with constraint A + B is either
# of equal size of smaller than a selection of the same data with just constraint A)
# we can make sure that everything has the same sample size.

# we want all three groups to have the same size
# this is not actually important (since we don't investigate standard errors)
# but reviewers might not be aware of this - note, however, that this selecting may also
...
## A function just to analyse DGP2, varying the proportion discon
